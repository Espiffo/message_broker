...
def run():
    with grpc.insecure_channel('localhost:50051') as channel:
        ...
        # Suscribirse a un canal y esperar mensajes
        def listen_for_messages():
            try:
                for message in stub.Subscribe(pubsub_pb2.Channel(name="test")):
                    print(f"Received message on channel 'test': {message.content}")
        ...
        while True:
            ...
            if message_content.lower() == 'exit':
                break
            stub.Publish(pubsub_pb2.Message(channel="test", content=message_content))

...
class PubSubService(pubsub_pb2_grpc.PubSubServicer):
    ...
    def __init__(self):
        ...
        self.subscribers = {}
        self.locks = {}  # Diccionario para almacenar los locks por canal
        self.semaphores = {}  # Diccionario para almacenar los semáforos por canal
    ...
    def _get_lock(self, channel):  # Obtener el lock para un canal
        if channel not in self.locks:
            # Usar un lock de nivel superior para evitar condiciones de carrera al crear locks
            with threading.Lock():
                if channel not in self.locks:
    ...
    def _get_semaphore(self, channel):  # Obtener el semáforo para un canal
        if channel not in self.semaphores:
            # Usar un lock de nivel superior para evitar condiciones de carrera al crear semáforos
            with threading.Lock():
                if channel not in self.semaphores:
                    self.semaphores[channel] = threading.Semaphore(100)  # Capacidad máxima de la cola
    ...
    def Publish(self, request, context):
        ...
        with lock:
            if channel not in self.channels:
                self.channels[channel] = queue.Queue(maxsize=100)  # Cola de mensajes por canal
            if channel not in self.subscribers:
                self.subscribers[channel] = []
            message_queue = self.channels[channel]
...
class PubSubService(pubsub_pb2_grpc.PubSubServicer):
    ...
    def __init__(self):
        ...
        self.channel_messages = {channel.name: queue.Queue(maxsize=100) for channel in self.channels} # Diccionario para mapear las colas de mensajes
